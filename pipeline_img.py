...
class PipelineImg:
    ...
    def do_full_pipeline(img, height=900, auto_canny_sigma=0.33, bordersize=15,*,debug=False, figsize=(10, 10)):
        ...
        unb_img = PipelineImg.drop_border_img(den_img, original=res_img, sigma=auto_canny_sigma, bordersize=bordersize)
    ...
    def drop_border_img(img, original, sigma=0.33, bordersize=15):
        ...
        crop_img = img[max(0, y-bordersize):min(y+h+bordersize, original.shape[0]-1), max(x-bordersize,0):min(x+w+bordersize,original.shape[1]-1)]
        return crop_img
